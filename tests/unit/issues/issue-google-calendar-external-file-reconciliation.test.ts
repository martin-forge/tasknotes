...
describe("Google Calendar external file reconciliation", () => {
	...
	it("syncs a direct external move of a recurring occurrence as an exception", async () => {
		const frontmatter: Record<string, unknown> = {};
		const originalTask = {
			path: "TaskNotes/Tasks/order-groceries.md",
		...
		const pluginData: Record<string, unknown> = {};
		const plugin = createPlugin([movedTask], frontmatter, pluginData);
		const googleCalendarService = createGoogleCalendarService();
		const syncService = new TaskCalendarSyncService(plugin, googleCalendarService as any);
		pluginData.googleCalendarTaskFingerprints = {
			[originalTask.path]: (syncService as any).getCalendarRelevantFingerprint(originalTask),
		};

		await syncService.handleExternalTaskFileUpdated(movedTask.path, movedTask);

		expect(frontmatter.googleCalendarExceptionOriginalScheduled).toBe("2026-05-23");
		expect(googleCalendarService.updateEvent).toHaveBeenCalledWith(
			"primary",
			"master-event-id",
			expect.objectContaining({
				recurrence: expect.arrayContaining(["EXDATE;VALUE=DATE:20260523"]),
			})
		);
		expect(googleCalendarService.createEvent).toHaveBeenCalledWith(
			"primary",
			expect.objectContaining({
				summary: "Order groceries",
