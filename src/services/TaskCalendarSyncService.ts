...
export class TaskCalendarSyncService {
	...
	private getTaskStateFromFingerprint(
		...
		return {
			...
			googleCalendarExceptionOriginalScheduled: stringValue(
				"googleCalendarExceptionOriginalScheduled"
			),
			googleCalendarMovedOriginalDates: stringArrayValue(
		...
		for (const task of tasks) {
			...
			if (this.hasTaskCalendarLink(task) && previousFingerprint !== fingerprint) {
				if (settings.syncOnTaskUpdate) {
					const taskForSync = await this.prepareTaskForExternalCalendarSync(
						task,
						previousTask
					);
					await this.executeTaskUpdate(taskForSync, previousTask);
				} else {
					fingerprints.set(task.path, fingerprint);
					changed = true;
				}
	...
	private async prepareTaskForExternalCalendarSync(
		task: TaskInfo,
		previousTask?: TaskInfo
	): Promise<TaskInfo> {
		if (!previousTask) {
		...
		applyGoogleCalendarRecurringExceptionCleanup(taskForSync);

		if (
			taskForSync.googleCalendarExceptionOriginalScheduled !==
				task.googleCalendarExceptionOriginalScheduled ||
			JSON.stringify(taskForSync.googleCalendarMovedOriginalDates || []) !==
				JSON.stringify(task.googleCalendarMovedOriginalDates || [])
		) {
			await this.saveTaskExceptionMetadata(task.path, {
				googleCalendarExceptionOriginalScheduled:
					taskForSync.googleCalendarExceptionOriginalScheduled,
				googleCalendarMovedOriginalDates: taskForSync.googleCalendarMovedOriginalDates,
			});
		}
